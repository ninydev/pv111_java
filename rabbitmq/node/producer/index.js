...
amqp.connect(RABBITMQ_CONNECTION_URI, {},
    async (errorConnect, connection) => {
        ...
        await connection.createChannel(async (errorChannel, channel) => {
            ...
            await channel.assertQueue('bus', { durable: true }, (errorQueue) => {
                ...
                setInterval( () => {
                    ...
                    channel.sendToQueue('bus', Buffer.from(JSON.stringify(bus)));
        ...
        await connection.createChannel(async (errorChannel, channel) => {
            ...
            console.debug("createChannel for car RabbitMQ ok");

            /**
             * Step 3
             * Assert channel to queue
             */
            await channel.assertQueue('car', {}, (errorEmailQueue) => {
                if (errorEmailQueue) {
                    console.error(errorEmailQueue);
                    process.exit(-1);
                }
                console.debug("Car queue asserted");
            });
        })
        ...
        await connection.createChannel(async (errorChannel, channel) => {
            ...
            console.debug("createChannel for track RabbitMQ ok");

            /**
             * Step 3
             * Assert channel to queue
             */
            await channel.assertQueue('track', {}, (errorEmailQueue) => {
                if (errorEmailQueue) {
                    console.error(errorEmailQueue);
                    process.exit(-1);
                }
                console.debug("Track queue asserted");
            });
        })
})